...
#### Cleaned R code for the Sensitivity analyses: Two-way fixed effect model                                                              ####
...
for (AP in c(mix_name, "PM25")){
  formula <- as.formula(paste0("y ~ ", AP, " + temp1 + temp2 + temp3 + temp4 + temp5 | year_month + GEOID"))
  model.fix2 <- feglm(formula, data = model.data, family = "gaussian", cluster = "GEOID")
  result <- as.data.frame(t(model.fix2[["coeftable"]][1,]))
  result[,1:2] <- result[,1:2]*IQR(model.data[,AP], na.rm = TRUE)
  all.result <- rbind(all.result, result)
}
row.names(all.result) <- c(mix_name, "PM25")
print(all.result)
print(Sys.time()-time.start)
saveRDS(result, file = paste0(save.path, "result_sa_TWFE.rds"))